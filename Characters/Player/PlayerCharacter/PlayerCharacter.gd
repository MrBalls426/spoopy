...
func _physics_process(delta: float) -> void:
	...
	if !ReconToggle and input_enabled: 
	...
	if Input.is_action_just_pressed("jump") and is_on_floor() and not ReconToggle and input_enabled:
		velocity.y = sqrt(jump_height * 2.0 * gravity)
		
	if !Input.is_action_pressed("crouch"):
		if Input.is_action_pressed("sprint") and is_on_floor() and not ReconToggle and input_enabled:
			velocity.x = direction.x * SPEED * sprint_speed
			velocity.z = direction.z * SPEED * sprint_speed
	
	if !Input.is_action_pressed("sprint"):
		if Input.is_action_pressed("crouch") and not ReconToggle and input_enabled:
